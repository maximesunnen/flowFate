...
n_datasets <- function(filename, ...) {
	...
	#flowCore:::findOffsets(con = con, keyword = "$NEXTDATA", emptyValue = FALSE)
...
# initialize a vector for the well identifiers (extracted from key-value pairs)
# Never grow a vector !!
#well_id <- c()
...
walk(seq_len(nb_ds), \(x) {
	fr <- read.FCS(filename,
								 ...
								 alter.names = TRUE,
								 emptyValue = FALSE)
	#well_id[x] <- fr[x]@description$`$WELLID`
...
FACS machines typically have predefined channel names that are not always identical to the fluorophores you're analyzing. Even worse, channel names are not consistent across FACS machines from different manufacturers. We therefore greatly facilitate our analysis by changing these channel names to the fluorophores we're analyzing. By running *colnames(fs)* in the command line you get a list of your FACS machine's channel names. 

On the left side of the code below, you write (between "") the name of the channel you want to change. On the right side, you write (between "") the name you want to change it to. In the first line, we change the channel name *GRN.B.HLin* to *GFP*. Delete the lines that are not necessary. Skip this code block if you don't want to perform any changes to your channel names.
```{r}
colnames(fs)[colnames(fs) == "GRN.B.HLin"]  <- "GFP"
colnames(fs)[colnames(fs) == "FSC.HLin"]    <- "FSC"
colnames(fs)[colnames(fs) == "SSC.HLin"]    <- "SSC"
colnames(fs)[colnames(fs) == "RED.R.HLin"]  <- "APC"
```
