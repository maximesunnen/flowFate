...
[Shiny apps](https://shiny.posit.co/) are a great feature of using R interactively.
However, each app is using a different set of packages, and even maybe a different R version.

to deal with this, one solution is to use [ShinyProxy](https://www.shinyproxy.io/) on our own [Shiny Server](https://posit.co/products/open-source/shinyserver/).

Then, each app is a docker image with its own set of packages. Each time an user is 
triggering an app, a container from the image is spawn from the appropriate image.
