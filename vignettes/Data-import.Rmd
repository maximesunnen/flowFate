...
Our analysis start with data import. Some flow cytometry acquisition software export data from different samples as **separate** .fcs files. Others export a **single, merged** .fcs file containing data multiple samples. The $NXTDATA keyword can then be used to retrieve data from individual samples. In flowFate, users can choose if they want to upload a merged file or individual .fcs files. In the latter case, these files must be grouped inside a folder.
...
We add the gatingSet, flowSet and nb_ds reactive expression to the reactive list. We're now able to use these variables in other modules by calling ```r$gs```, ```r$fs``` or ```r$nb_ds```. These assignments need to be wrapped inside observe() in order to read Shiny reactive expressions.
